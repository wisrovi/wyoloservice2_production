...
The core objective of this system is to decouple the user interface layer from the heavy-compute layer. This entire orchestration and its underlying capabilities are achieved using the **wisrovi-suit** (https://github.com/wisrovi/w-cli). It enables researchers and developers to submit simple YAML configurations to a centralized cluster. The cluster autonomously handles load balancing, priority assignment, model evolution (mutations through genetic algorithms), and structured metric logging.
...
| ID | Feature / Dimension | Train Service 1 (Legacy / "Teenager Guard") | Train Service 2 (Modern / "The Terminator") |
| :---: | :--- | :--- | :--- |
| **01** | **Resource Isolation & Allocation** | **Monopolistic:** The worker process hogged all host RAM, CPU, and GPU resources across all trials, locking the machine completely. | **Single-Trial Concurrency:** Only occupies the host for a single trial (`--concurrency=1`), auto-releasing GPU/RAM and clean reaping zombie processes after each round. |
| **02** | **Hyperparameter Search Strategy** | **Basic & Random:** Parameter exploration was purely random and confined exclusively to the static bounds of the trials. | **Genetic HPO Engine:** Driven by genetic algorithms trained on vast historical YOLO runs, selecting optimal parameters based on model history (not just current trials). |
| **03** | **Historical Optimization Context** | **Memoryless:** No memory of past runs. Failed to leverage historical days or previous trial results to guide new searches. | **History-Aware:** Relies on a persistent PostgreSQL backend tracking all studies and trials. Uses historical data to continuously guide and optimize new parameter sweeps. |
| **04** | **Framework & Model Support** | **Rigid:** Lacked native updates or integration for newer architectures, causing immediate failures with newer versions. | **Universal Framework Support:** Fully and natively supports all YOLO updates, including advanced vision models like YOLO26 out-of-the-box. |
| **05** | **Storage & Datastore Stability** | **Volatile Local Storage:** Disk space filled up continuously due to unbounded MLflow metrics and datasets, causing catastrophic OS lockups. | **Smart Decoupled Storage:** Centralized MinIO S3 and PostgreSQL integration with autonomous rules defining what artifacts to keep or automatically purge. |
| **06** | **Resource Scaling Limits** | **Hard-Capped:** Strictly restricted to whatever hardware was shared locally; incapable of distributed scaling. | **Virtual Resource Pooling:** Pools resources across nodes. If 10 nodes share 1GB GPU/RAM each, a user can train a single large 9GB GPU model dynamically. |
| **07** | **Documentation & Telemetry Summaries** | **Manual:** Documentation (EDA and post-training analysis) was written entirely by the user. | **Automated Insights:** Automatically generates and streams EDA graphs, confusion matrices, F1 curves, and metrics directly to MLflow and MinIO. |
| **08** | **Configuration Safety** | **Brittle & Silent Failures:** A single syntax error (like an extra space in the YAML file) resulted in immediate silent training failures. | **Pre-flight Compilation & Verification:** Integrated Model Context Protocol (MCP) and schemas validate the config structure and report clear diagnostics. |
| **09** | **GPU Driver Connections** | **Unstable:** High risk of losing connection to the GPU after execution, requiring physical machine reboots. | **Resilient Driver Hooking:** Automatically mounts and hooks the GPU cleanly every time, assuming the driver itself is without fatal errors. |
| **10** | **System Stability & Zombie Cleanup** | **Fragile Host State:** Resources routinely hung. Zombie processes and GPU/RAM leaks forced physical machine reboots. | **Indestructible Watchdogs:** Systemd Watchdog services monitor invokers, containerizing runtime processes to prevent system-wide memory or thread leaks. |
| **11** | **Samba Multi-OS Sharing** | **Brittle & Fragile:** Sharing Samba paths between different operating systems (Linux/Windows) was highly unstable and prone to crash mid-trial. | **Hardened Mount Controls:** Automated scripts mounting CIFS shares via secure, restricted credential files (`chmod 600`) with high cross-OS resilience. |
| **12** | **AI Agentic Integration (MCP)** | `--` | **Native FastMCP Server:** Exposes custom tools allowing AI agents (like Antigravity) to monitor status, validate datasets, and launch sweeps using natural language. |
| **13** | **User Interface (UI)** | `--` | **WDarwin Ops SPA:** Modern React UI dashboard showing real-time host telemetry, remote file exploration, and visual study histories. |
| **14** | **Auto-Updates (CI/CD)** | `--` | **Watchtower Integration:** Autonomous hot-reloading checks Docker Hub every 10 minutes and updates active worker nodes with zero downtime. |
| **15** | **Study Resiliency & Recovery** | `--` | **State Persistence:** PostgreSQL-backed state machine. Studies can be paused, resumed, or recovered seamlessly after system blackouts or master crashes. |
| **16** | **Remote Integration Gateway** | `--` | **FastAPI Gateway:** Exposes REST API endpoints allowing any remote service to submit, monitor, and cancel training runs. |
| **17** | **Task Priority Routing** | `--` | **Multi-Priority Routing:** Celery-driven task queue with dynamic priority levels (`gpus_high`, `gpus_medium`, `gpus_low`) and concurrent execution load-balancing. |
| **18** | **Integrated Smoke Tests Suite** | `--` | **1-Click Smoke Test Runner:** GUI-triggered dry-runs or advanced E2E YOLO26 multi-task pipelines (Classify, Detect, Segment) for immediate validation. |
| **19** | **Pre-flight Dataset Verification** | `--` | **Dataset Auto-Repair Engine:** Runs pre-flight checks on datasets and automatically triggers repairs/auto-reparation before initiating YOLO sweeps. |
| **20** | **Dynamic Auto-Batching Adaptation** | `--` | **Dynamic Auto-Batching:** Utilizing `batch: -1` leverages YOLO’s internal hardware-capacity engine to auto-scale batches up to 90% VRAM safety. |
| **21** | **Automated Installation Wrapper** | `--` | **1-Click Setup Script:** Executable installation package wraps all services, configurations, watchdogs, and mount structures in one command. |
| **22** | **Container Shared Memory Tuning** | `--` | **Automatic SHM Allocation:** Automatically claims and configures shared memory boundaries (e.g. `--shm 24G`) preventing PyTorch dataloader crashes. |
| **23** | **Optuna Dashboard Integration** | `--` | **Direct PG Database Visualizer:** Exposes study database tables to Optuna Dashboard in real-time, allowing advanced analytical parameter plotting. |
| **24** | **Centralized Study Management** | `--` | **Celery Study Manager:** A dedicated background daemon orchestrating genetic study mutations and task routing, decoupling state from compute. |
| **25** | **Real-time Host Telemetry** | `--` | **Dynamic Telemetry Daemon:** Ingests live hardware status (CPU, RAM, Disk Space) on active GPU workers and streams it to the UI. |
| **26** | **Samba Credentials Hardening** | `--` | **Encapsulated Security:** Encrypted credential files (`chmod 600`) restricted exclusively to root ownership, hiding passwords from configurations. |
...
| **42** | **Onion-Layer Architecture Extensibility** | `--` | **Extensible Execution Layers:** Allows attaching custom execution steps before, during, or after training sweeps (e.g. running stats, notifications). |
| **43** | **Automated EDA Document Layer** | `--` | **Onion Layer EDA:** Automatic execution layer that builds and generates a comprehensive Exploratory Data Analysis report. |
| **44** | **vLM-based Dataset Auto-Descriptions** | `--` | **Onion Layer vLM:** Evaluates dataset samples with Vision-Language Models to automatically generate self-descriptive reports. |
| **45** | **Explainable AI Activations (Grad-CAM)** | `--` | **Grad-CAM Activation Layer:** Runs post-training analysis visualizing exact features the model pays attention to (e.g. orejas instead of head), prompting dataset augmentation. |
| **46** | **Urgent Queue Bypass** | `--` | **Priority Queue Escalation:** Allows urgent runs to bypass queues of 100+ tasks, seizing the first available computing node. |
| **47** | **Cloud Native Elastic Execution** | `--` | **Docker & Kubernetes (K8s):** Fully supports deployment under Kubernetes node clusters with horizontal auto-scaling capability. |
