...
| **AI Agentic Integration (MCP)** | **None / Impossible:** No mechanism or protocol for AI agents to interact with the system. Only manual human control. | **Native FastMCP Server:** Exposes custom tools allowing AI agents (like Antigravity) to monitor status, validate datasets, and launch sweeps. |
| **User Interface (UI)** | **Non-existent:** Purely terminal CLI execution. No visual metrics, remote file exploration, or status checks. | **WDarwin Ops SPA:** Modern React UI dashboard showing real-time host telemetry, remote file exploration, and visual study histories. |
| **Auto-Updates (CI/CD)** | **Manual Interruption:** Any software update required manually SSHing into nodes, killing processes, and pulling/building code. | **Watchtower Integration:** Autonomous hot-reloading checks Docker Hub every 10 minutes and updates active worker nodes with zero downtime. |
| **Study Resiliency** | **Zero Recovery:** Rebooting or power outage during a sweep erased all trial memory, requiring a complete restart. | **State Persistence:** PostgreSQL-backed state machine. Studies can be paused, resumed, or recovered seamlessly after system blackouts. |
| **Remote Integration Gateway** | **Command-Line Bound:** Restricted only to local execution, blocking integrations with external pipelines. | **FastAPI Gateway:** Exposes REST API endpoints allowing any remote service to submit, monitor, and cancel training runs. |
