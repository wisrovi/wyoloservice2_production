...
> [!IMPORTANT]
> The physical Docker Hub image tag must ALWAYS be strictly kept as **`wisrovi/train_service:worker_invoker_v1.0.0`** to allow **Watchtower** to trigger updates on all 70+ node installations. Software releases and logical versioning (e.g. current release **`v1.3.2`**) are updated internally within the image, but published under the same physical tag.
